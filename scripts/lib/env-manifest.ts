...
export const ENV_VARS: EnvVar[] = [
  ...
  // --- admin / signup ------------------------------------------------------
  {
    ...
    prompt: "Enable public signup",
    hint: "When disabled, production bootstrap requires ADMIN_EMAIL; development and test allow the first user to become admin.",
    default: "false",
    example: "false",
    comment: [
      "DISABLED by default. In development/test, the first signup on an empty",
      "instance becomes admin. In production with this false, set ADMIN_EMAIL",
      "before first use; only that owner may bootstrap an admin. SMTP is NOT",
      "required for signup — without SMTP, accounts are marked verified at create.",
    ],
  },
  {
    key: "ADMIN_EMAIL",
    group: "admin",
    source: "prompt",
    prompt: "Production bootstrap admin email (blank when public signup is enabled)",
    hint: "Required for a fresh production database when public signup is disabled.",
    example: "operator@example.com",
    comment: [
      "Canonical owner email for the first production admin when SIGNUP_ENABLED=false.",
      "Matching is case-insensitive and whitespace is ignored; one address is permitted.",
      "Required before a fresh production database can bootstrap an admin.",
    ],
  },
  ...
  {
    key: "RATE_LIMIT_SIGNIN_FAILURE_POINTS",
    group: "ratelimit",
    source: "default",
    default: "10",
    comment: ["Failed email/password checks per account; applies across rotating client IPs."],
  },
  {
    key: "RATE_LIMIT_SIGNIN_FAILURE_DURATION",
    group: "ratelimit",
    source: "default",
    default: "900",
  },
  {
    key: "RATE_LIMIT_SIGNIN_FAILURE_BLOCK_DURATION",
    group: "ratelimit",
    source: "default",
    default: "600",
  },
