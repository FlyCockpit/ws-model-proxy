...
export class CanonicalStreamParser {
  ...
  readonly #errorMetadata: { status?: number; requestId?: string; retryAfter?: string };
  ...
  #chat(value: Record<string, unknown>): CanonicalEvent[] {
    ...
    if (typeof delta.content === "string" && delta.content)
      if (this.#items.has(0) && this.#itemTypes.get(0) !== "text")
    ...
    if (typeof delta.refusal === "string" && delta.refusal)
      if (this.#items.has(1) && this.#itemTypes.get(1) !== "refusal")
    ...
    if (choice.finish_reason != null) events.push(...this.#stop(stopReason(choice.finish_reason)));
  ...
  #responses(type: string, value: Record<string, unknown>): CanonicalEvent[] {
    ...
    if (value.sequence_number !== undefined) {
      ...
      if (!Number.isSafeInteger(sequence) || (sequence as number) <= this.#lastSequence)
        throw new AdapterError(
    ...
    if (type === "response.output_item.added") {
      ...
      if (Array.isArray(part.annotations) && part.annotations.length)
        unsupported("stream.data.part.annotations");
      if (Array.isArray(part.logprobs) && part.logprobs.length)
        unsupported("stream.data.part.logprobs");
      ...
      if (value.content_index !== undefined && value.content_index !== 0)
        throw new AdapterError("multiple_content_parts", "Only content index zero is adaptable.");
      ...
      if (this.#itemTypes.get(index) === "tool_call" && typeof item.arguments === "string") {
        ...
        this.#toolArguments.set(index, item.arguments);
      }
      ...
      events.push({
        type: "error",
        error: this.#streamError(value.error ?? response.error),
      });
    }
    return events;
  }
  ...
  #anthropic(type: string, value: Record<string, unknown>): CanonicalEvent[] {
    ...
    if (type === "message_start") {
      const initialUsage = usageEvent(message.usage);
      if (initialUsage) events.push(initialUsage);
    }
...
function responsesFields(type: string): readonly string[] {
  if (type === "error") return ["type", "error", "sequence_number"];
...
export class CanonicalStreamRenderer {
  ...
  constructor(surface: ProtocolSurface, options: { signal?: AbortSignal } = {}) {
    this.#surface = surface;
    this.#signal = options.signal;
  }
  ...
  #responses(event: CanonicalEvent): WirePayload[] {
    ...
    if (event.type === "complete")
      return [
        named(this.#stopEvent?.reason === "length" ? "response.incomplete" : "response.completed", {
          response: {
            id: this.#messageId,
            object: "response",
    ...
    if (event.type === "error")
      return [{ event: "error", data: renderProtocolError("openai-responses", event.error) }];
    if (event.type === "reasoning_delta")
      throw new AdapterError(
        "unsupported_stream_event",
  ...
  #anthropic(event: CanonicalEvent): WirePayload[] {
    ...
    if (event.type === "item_complete")
      return [named("content_block_stop", { index: event.index })];
    if (event.type === "usage")
      return [
        named("message_delta", {
          delta: {},
...
export function createCanonicalSseTransform(
  surface: ProtocolSurface,
  options: { signal?: AbortSignal; highWaterMarkBytes?: number; maxChunkBytes?: number } = {},
