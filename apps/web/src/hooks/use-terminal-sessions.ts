...
export function useTerminalSessions(): {
    ...
    (message: TerminalServerMessage) => {
      if (message.type === "terminals") {
        ...
        for (const remote of message.terminals) {
          const known = tabsRef.current.some((tab) => tab.terminalId === remote.terminalId);
          if (known || additions.some((tab) => tab.terminalId === remote.terminalId)) continue;
          additions.push({
            localId: newId("local"),
      ...
      if (!message.terminalId) return;
      const localId = tabsRef.current.find((tab) => tab.terminalId === message.terminalId)?.localId;
      if (!localId) return;
  ...
  const onDisconnect = useCallback(() => {
    ...
    setTabs((current) =>
      current.map((tab) =>
        tab.phase === "live" || tab.phase === "opening"
          ? { ...tab, phase: "opening", error: null }
          : tab,
      ),
    );
  }, []);
    ...
    (frame: { terminalId: string; seq: number; body: Uint8Array }) => {
      ...
      void openTerminalBytes({
        ...
        .then((plaintext) => {
          ...
          if (decoded.kind === "data") emitOutput(session.localId, decoded.data);
        })
        .catch(() => undefined);
    },
    [emitOutput],
